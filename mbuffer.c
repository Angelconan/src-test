...
int listXsize[6];	//++ 偶数为前向参考帧列表中参考帧的个数，即list0；奇数为后项参考帧列表中参考帧的个数，即list1。
...
/*//++
  get_mem2Dint (&(s->slice_id), size_y / MB_BLOCK_SIZE, size_x / MB_BLOCK_SIZE);

  get_mem3Dint (&(s->ref_idx), 2, size_y / BLOCK_SIZE, size_x / BLOCK_SIZE);
  ...
  get_mem4Dint (&(s->mv), 2,2, size_x / BLOCK_SIZE, size_y / BLOCK_SIZE );
 ...
 ************************************************************************
 */
void init_lists(int currSliceType, PictureStructure currPicStructure) //瀵圭紪鐮佸浘鍍忕紦瀛樹腑鐨勫凡缂栫爜鍥惧儚鐨勫弬鑰冨抚缂栧彿杩涜鍒濆鍖?
{
  ...
  if (currPicStructure == FRAME)  
  {
    ...
    {
      if (dpb.fs_ref[i]->is_used==3)	//++ 如果当前图像为帧，则只有帧、互补参考场对才会被用于参考，因此只对这样的帧缓冲进行frame_num_wrap计算
      {
        if ((dpb.fs_ref[i]->frame->used_for_reference)&&(!dpb.fs_ref[i]->frame->is_long_term))	//++ 第二个条件判断说明如果一个帧缓冲中两个场为一个长期参考一短期参考，则该帧缓冲也在短期参考帧缓冲列表中，但其被标记为长期参考
        {
          ...
          dpb.fs_ref[i]->frame->pic_num = dpb.fs_ref[i]->frame_num_wrap;	//++ 由此可见帧模式下pic_num与frame_num_wrap两值相等（标准，公式8-29）
...
//      printf("listX[0] (PicNum): "); for (i=0; i<list0idx; i++){printf ("%d  ", listX[0][i]->pic_num);} printf("\n");
    ...
    else
    {
      ...
      qsort((void *)fs_list0, list0idx, sizeof(FrameStore*), compare_fs_by_frame_num_desc);	//++ 因为场模式下，frame_num_wrap标志的是一帧图像，而同一帧的两个场
																							//++ 不应该被隔开，所以可以直接采用frame_num_wrap对参考图像进行排序
...
//      printf("listX[0] (PicNum): "); for (i=0; i<listXsize[0]; i++){printf ("%d  ", listX[0][i]->pic_num);} printf("\n");
  ...
  {
    // B-Slice
    if (currPicStructure == FRAME)  //++ 参见标准8.2.4.2.1、8.2.4.2.3
    {
      for (i=0; i<dpb.ref_frames_in_buffer; i++)
      {
        if (dpb.fs_ref[i]->is_used==3)
...
ColocatedParams* alloc_colocated(int size_x, int size_y, int mb_adaptive_frame_field_flag)
{
  ...
  get_mem3Dint (&(s->ref_idx), 2, size_x / BLOCK_SIZE, size_y / BLOCK_SIZE);	//++ 为直接模式中要用到的每个8*8相对位置块分配参考列表
...
void compute_collocated(ColocatedParams* p, StorablePicture **listX[6])
{
  ...
  fs_top=fs_bottom=fs = listX[LIST_1 ][0];	//++ 将B片后面（显示顺序）的图像作为“相对位置图像”（这里是先赋给了fs，程序在后面会将fs再赋值给“相对位置图像”）。
											//++ 因为“相对位置分割（co_located partition）”位于该参考帧中，所以我称该图像为“相对位置图像”，参见标准图8-2。
...
//      if ((img->structure != fs->structure) && ((fs->MbaffFrameFlag) || ((!fs->MbaffFrameFlag)&&(fs->coded_frame))))
  ...
  if (img->direct_type ==0)	//++ 如果图像中的直接模式是采用时间直接模式编码
  {
    for (j=0 ; j<fs->size_y/4 ; j++)      
    {                
      for (i=0 ; i<fs->size_x/4 ; i++)          
    ...
    for (j=0; j<2 + (img->MbaffFrameFlag * 4);j+=2)	//++ 根据POC计算向量缩放比例
    {
