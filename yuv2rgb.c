...
bool YUV2RGB(LPBYTE yBuf, LPBYTE uBuf, LPBYTE vBuf, int size_x, int size_y, int size_x_cr, int size_y_cr, int crop_top, int crop_left, int crop_bottom, int crop_right)
{
	...
	//printf("data.txt is ok1!");
	//#define PIXELSIZE nWidth * nHeight  
	//printf("data.txt is ok5!\n");
	...
	//if (!yBuf || !pRgbBuf)
	//	return false;
	//printf("data.txt is ok4!\n");
	//long nYLen = nWidth * nHeight;
	//int nHfWidth = (nWidth >> 1);
	//if (nYLen<1 || nHfWidth<1)
	//	return false;
	//printf("data.txt is ok3!\n");
	// Y data  
	unsigned char** yData = yBuf;
	// v data  
	unsigned char** vData = uBuf;
	// u data  
	unsigned char** uData = vBuf;
	//printf("data.txt is ok2!\n");
	if (!uData || !vData)
		return false;
	int rgb[3];
	...
	bool addhalf = true;
	fprintf(fp, "%d ", 999);
	for (y = crop_top; y<size_y - crop_bottom; y++) {
		//	m += nWidth;
		if (addhalf) {
			//		n += nHfWidth;
			addhalf = false;
		...
		for (x = crop_left; x<size_x - crop_right; x++) {
			py = yData[y][x];
			//printf("TELL ME WHY \n");
			//printf("%d\n",yData[y][x]);
			//printf("%d\n", vData[y/2][x/2]);
			// search tables to get rdif invgdif and bidif  
			rdif = Table_fv1[vData[y >> 1][x >> 1]];    // fv1  
			invgdif = Table_fu1[uData[y >> 1][x >> 1]] + Table_fv2[vData[y >> 1][x >> 1]]; // fu1+fv2  
			bdif = Table_fu2[uData[y >> 1][x >> 1]]; // fu2       
			rgb[1] = py + rdif;    // R  
			rgb[2] = py - invgdif; // G  
			rgb[0] = py + bdif;    // B  
			...
			for (j = 0; j<3; j++)
			{
				if (rgb[j] >= 0 && rgb[j] <= 255) {
					...
					pRgbBuf = (rgb[j] < 0) ? 0 : 255;
				}
				//printf("%d\n", pRgbBuf);
				fprintf(fp, "%d ", pRgbBuf);
			}
		}
		fprintf(fp, "%d ", 998);
	}
