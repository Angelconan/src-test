...
typedef struct macroblock
{
  ...
  int           mb_type;	//++ 当前宏块类型。对于P_Skip和B_Skip类型宏块，该值不从码流中解析，而直接被附值为0；对于帧内片以及其余帧间（包括P、B）片的宏块类型，该值从码流中解析得到；
							//++ 对于P宏块，该值=码流解析值+1；对于B宏块，该值=码流解析值（具体体现在 macroblock.c 文件中 if(img->type == P_SLICE || img->type == SP_SLICE)语句）。
  int           mvd[2][BLOCK_MULTIPLE][BLOCK_MULTIPLE][2];      //!< indices correspond to [forw,backw][block_y][block_x][x,y]
  int           cbp, cbp_blk ; // CBP琛ㄧず娈嬪樊鐨勭紪鐮佺姸鎬?CBP涓�鍏?bit锛屼綆4浣嶈〃绀?涓寒搴?x8鍧?绗?浣嶈〃绀篣,绗簲浣嶈〃绀篤,濡傛灉鐩稿簲鐨勪綅涓?1", 琛ㄧず姝ゅ潡鏈夋畫宸郴鏁?鍙嶄箣娌℃湁娈嬪樊,姝ゅ畯鍧楁病鏈夎缂栫爜.
  unsigned long cbp_bits;

  int           is_skip;

  int           i16mode;	//++ 当宏块类型为帧内16*16时16*16亮度块的预测模式
  int           b8mode[4];	//++ 每个8*8块的分割模式
  int           b8pdir[4];	//++ 8*8块的的预测方向：-1=帧内编码，0=前向预测，1=后向预测，2=双向预测
  int           ei_flag;
...
typedef struct datapartition
{
  ...
  int     (*readSyntaxElement)(SyntaxElement *, struct img_par *, struct inp_par *, struct datapartition *); //璇诲彇鍙ユ硶鍏冪礌
          /*!< virtual function;
...
typedef struct
{
  ...
  PictureStructure    structure;     //!< Identify picture structure type 甯ф垨鍦?
...
{
  int number;                                 //!< frame number	//++ 解码的图像数（不包括B图像）
  unsigned current_mb_nr; // bitstream order	//++ 当前宏块的宏块号
  unsigned num_dec_mb;
  int current_slice_nr;
  int *intra_block;							//++ 用来表示每个宏块是否按帧内方式编码
  ...
  int type;                                   //!< image type INTER/INTRA 閲囩敤甯ч棿缂栫爜鎴栬�呭抚鍐呯紪鐮?
  int width;
  int height;
  ...
  int m7[16][16];								//!< final 4x4 block. Extended to 16x16 for ABT	//++ 存放逆变换完的结果,一般是预测残差值
  int cof[4][6][4][4];							//!< correction coefficients from predicted
  int cofu[4];
  int **ipredmode;								//!< prediction type																																		//	Y Y Y Y
  int quad[256];																																															//	Y Y Y Y
  int ***nz_coeff;								//!< number of coefficients per block	//++ 每个4*4块拥有的非零系数个数（对于色度块以及intra16*16类型宏块的亮度块，不包括直流系数），6*4的矩阵中，放置顺序为：	Y Y Y Y
  int **siblock;																																															//	Y Y Y Y
  int cod_counter;								//!< Current count of number of skipped macroblocks in a row 浠庡綋鍓嶅畯鍧楀紑濮嬭繛缁嚭鐜扮殑Skip瀹忓潡鐨勬暟閲?	//++ 从当前宏块开始连续出现的Skip宏块的数量													//	U U V V
																																																			//	U U V V
  int newframe;
  ...
  // For MB level frame/field coding
  int MbaffFrameFlag; //1涓哄抚鍦鸿嚜閫傚簲锛?涓嶆槸
  ...
  unsigned int frame_num;   //frame_num for this frame	//++ 帧号：参考帧帧号按解码顺序递增，非参考帧与后续参考帧使用同一帧号
  ...
  int idr_pic_id;  //IDR鍥惧儚鏍囪瘑鍊硷紝涓嶅悓鐨処DR鍥惧儚鎷ユ湁涓嶅悓鐨処DR鍊?

  int MaxFrameNum;

  unsigned PicWidthInMbs;	//++ 图像宽度（以宏块为单位计算）
  unsigned PicHeightInMapUnits;	//++图像高度（以宏块为单位计算）
  unsigned FrameHeightInMbs;	//++帧高度（以宏块为单位计算）
...
typedef struct pix_pos
{
  ...
  int x;		//++ 像素在本宏块中的位置横坐标
  int y;		//++ 像素在本宏块中的位置纵坐标
  int pos_x;	//++ 像素在帧中的位置横坐标
  int pos_y;	//++ 像素在帧中的位置纵坐标
} PixelPos;
