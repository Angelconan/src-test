...
void write_out_picture(StorablePicture *p, FILE *p_out)
{
  ...
  #ifdef distinguish_target
	distinguish_part_location_uv(i,j,p,crop_top,crop_bottom,crop_left,crop_right,p_out);
  #else
...
{
  out_buffer = alloc_frame_store(); //为一帧图像索引分配内存空间，并初始化相关PPS、SPS相关参数
  pending_output = calloc (sizeof(StorablePicture), 1); //为码流中的图像分配存储空间
  if (NULL==pending_output) no_mem_exit("init_out_buffer");
  pending_output->imgUV = NULL; //初始化色度参数为null
  pending_output->imgY  = NULL; //初始化亮度参数为null
}
