...
typedef struct storable_picture
{
  ...
  unsigned    frame_num;
  int         pic_num;
  int64       ref_pic_num        [MAX_NUM_SLICES][6][MAX_LIST_SIZE];	//++ �ο�ͼ���ͼ���
  ...
  int  ***    ref_idx;       //!< reference picture   [list][subblock_x][subblock_y]	//++ �ο�ͼ����ڲο�ͼ���б��е������������б��е�λ��
...
//  byte *      mb_field;      //!< field macroblock indicator
  ...
  int  ***    ref_idx;       //!< reference picture   [list][subblock_x][subblock_y]	//++ �ο�ͼ����ڲο�ͼ���б��е������������б��е�λ��
  int64 ***    ref_pic_id;    //!< reference picture identifier [list][subblock_x][subblock_y]	//++ �ο�ͼ���ͼ���
  int  ****   mv;            //!< motion vector       [list][subblock_x][subblock_y][component]  
  byte **     moving_block;
...
{
  FrameStore  **fs; //参考图像列表，该指针主要用于管理一帧图像，期中包括一个帧与两个场，其成员FRAME管理帧，top_feild与bottom_field回用于管理场
  FrameStore  **fs_ref; //短期参考图像列表（未经排序�?
  FrameStore  **fs_ltref; //长期参考图像列�?
  unsigned      size; //dpb足底啊容量，即最多容的下多少帧，最大不超过16
  unsigned      used_size; //dpb实际存了多少帧，以帧为单位，如果只有其中一场，也被看作一�?
  unsigned      ref_frames_in_buffer; //chj fs中用做short-term reference的帧的个数，短期参考帧，遇到下个I(IDR)会清空缓�?
