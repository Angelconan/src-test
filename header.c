...
int FirstPartOfSliceHeader()
{
  ...
  int dP_nr = assignSE2partition[currSlice->dp_mode][SE_HEADER]; //Slice澶寸殑DP鍊奸�夊彇
  DataPartition *partition = &(currSlice->partArr[dP_nr]); //绗竴鍒嗗尯鐨勫湴鍧�
  Bitstream *currStream = partition->bitstream;
  int tmp;

  UsedBits= partition->bitstream->frame_bitoffset; // 鍦║VLC缂栫爜涓嬬爜娴佺紦瀛樹腑鐨勫疄闄呬綅缃畐as hardcoded to 31 for previous start-code. This is better.
...
int RestOfSliceHeader()
{
  ...
  img->frame_num = u_v (active_sps->log2_max_frame_num_minus4 + 4, "SH: frame_num", currStream); //U(V)鏍规嵁搴忓垪鍙傛暟闆哠PS涓殑log2_max_frame_num_minus4+4鏉ョ‘瀹氬皢瑕佽鍙栫殑frame_num鍙ユ硶鍏冪礌鐨勭爜娴侀暱搴?

  /* Tian Dong: frame_num gap processing, if found */
  if (img->idr_flag) //濡傛灉涓篒DR鍥惧儚鍒欏弬鑰冨抚num浠?璁★紝骞朵笖瀹冪殑frame_num蹇呬负闆?
  {
    img->pre_frame_num = img->frame_num;
    assert(img->frame_num == 0);
  }

  if (active_sps->frame_mbs_only_flag) //鏈簭鍒楃殑鍥惧儚閮戒负甯э紵
  {
    img->structure = FRAME;
    img->field_pic_flag=0; //闈炲満缂栫爜锛屼篃鏄疭lice灞傛爣璇嗗浘鍍忕紪鐮佹ā寮忕殑鍞竴涓�涓彞娉曟ā寮?
  }
  else
  {
    // field_pic_flag   u(1)
  ...
  ref_pic_list_reordering();	//++ 参见标准7.3.3.1
  ...
  if (img->nal_reference_idc)
    dec_ref_pic_marking(currStream);	//++ 参见标准7.3.3.3
...
void decode_poc(struct img_par *img)
{
  ...
  case 0: // POC MODE 0
    ...
    if ((!img->field_pic_flag)||(!img->bottom_field_flag ))	//++ 当前为帧图像，或者为场图像的顶场
    {
      img->ThisPOC= img->toppoc = img->PicOrderCntMsb + img->pic_order_cnt_lsb;
    }

    if (!img->field_pic_flag)	//++ 当前为帧图像
    {
      img->bottompoc = img->toppoc + img->delta_pic_order_cnt_bottom;
    }
    else	//++ 当前图像为场图像
    {
      if( img->bottom_field_flag )	//++ 当前为场图像的低场
      {
