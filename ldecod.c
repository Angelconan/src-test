...
//extern candidate_list *candi_tracking;
...
//int active_zone_input[4] = { 44,96,12,12 }; //football
//int active_zone_input[4] = { 140,16,32,96 }; //football-red-man
//int active_zone_input[4] = { 88,120,32,24 }; //train-redball
//int active_zone_input[4] = { 128,112,48,32}; //train
//int active_zone_input[4] = { 48,48,64,64 };//man
//int active_zone_input[4] = { 128,16,32,112 };//CREW_man
//int active_zone_input[4] = { 96,0,32,80 };
//int active_zone_input[4] = { 112,32,48,48 };//tennis
//int active_zone_input[4] = { 48,0,80,208 };
  int active_zone_input[4] = { 196,64,52,128 };
/*!
***********************************************************************
* \brief
...
int main(int argc, char **argv)
{
	...
	init_conf(input, argv[1]);  //初始化configfile到内存空间，并向屏幕输出�?

	init_old_slice();  //slice初始�?

	switch (input->FileFormat)
	{
	case 0:
		OpenBitstreamFile(input->infile);  //得到bits文件的地址（H.264�?
		break;
	case 1:
	...
	malloc_slice(input, img); //为SPS指向的Slice申请存储空间，同时为解码的运动、纹理信息分配内存空间将首地址给Slice框架，同时完成数据分区，并为每个数据分区的码流分配存储空�?000000byte

	init(img); //SPS初始化，quad�?56�?

	dec_picture = NULL; //Storable_picture的一个实例化结构体未分配内存空间

	dpb.init_done = 0;  //图像解码的buffer已解码的图像�?
	g_nFrame = 0;

	//  init_dpb(input);
	init_out_buffer();

	img->idr_psnr_number = input->ref_offset; //idr峰值信噪比参考帧偏移量传值？�?
	img->psnr_number = 0;  //峰值信噪比？？

	img->number = 0;  //帧号
	img->type = I_SLICE; //赋值给Type的枚举索引号给type整型
	img->dec_ref_pic_marking_buffer = NULL;  //标记参考帧参数并未分配存储空间
	...
	char *config_filename)
{
	...
	if ((fd = fopen(config_filename, "r")) == NULL) //判断文件是否正常打开
	{
...
#if TRACE
	if ((p_trace = fopen(TRACEFILE, "w")) == 0)             //如果trace_dec存在则清零，如果不存在则创建 append new statistic at the end
	{
...
DataPartition *AllocPartition(int n)
{
	...
	{
		dataPart = &(partArr[i]); //指向第一个连续空间的首地址
		...
		dataPart->bitstream->streamBuffer = (byte *)calloc(MAX_CODED_FRAME_SIZE, sizeof(byte));	//申请800000个连续的字节空间来存储码�?
...
void malloc_slice(struct inp_par *inp, struct img_par *img)
{
	...
	img->currentSlice = (Slice *)calloc(1, sizeof(Slice)); //指向slice结构体的首地址，同时申请内存空间�?
	...
	if (1)
	{
		...
		currSlice->mot_ctx = create_contexts_MotionInfo(); //为解码相关运动信息MotionInfoContext分配内存空间，并返回首地址给Slice框架
		currSlice->tex_ctx = create_contexts_TextureInfo();//为解码相关纹理信息TextureInfoContext分配内存空间，并返回首地址给Slice框架
	}
	currSlice->max_part_nr = 3;  //! assume data partitioning (worst case) for the following mallocs()
	currSlice->partArr = AllocPartition(currSlice->max_part_nr);//数据分区，并返回数据分区的首地址，同时分配每个分区的码流存储空间
}
